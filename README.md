...
- 播放器控制栏新增可配置时长的快进/快退和 OP/ED 跳过按钮.
- 弹幕开关移动到播放器右下角控制栏.
- 导入/导出全部状态, 可组合登录信息与可导出设置.
- 我的页新增本地视频媒体库, 支持手动添加文件夹、自定义扫描后缀与无后缀/子文件夹/.nomedia 选项, 播放器可选集本地文件 (仅 Android).
- 视频播放器新增字幕浏览面板, 可查看当前字幕全部条目, 支持搜索并点击跳转到对应时间, 勾选跟随后当前字幕会保持在视野中.
- 直播流卡顿或中断时自动重连, 并重新请求播放地址, 不再复用可能已失效的旧链接.
- 直播弹幕消息改为分批让出处理, 并限制消息列表长度, 避免弹幕洪峰时长时间占用主线程导致界面无响应.
- Android 默认音频输出改为 AudioTrack 优先, 避免 OpenSL ES 落到独立 deep buffer 输出后与其它应用互抢全局音效链, 导致本应用播放音量被其它应用影响.
